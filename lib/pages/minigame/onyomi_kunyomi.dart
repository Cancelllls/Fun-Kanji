...
class _OnyomiKunyomiMinigameState extends State<OnyomiKunyomiMinigame>
  ...
  static const _storageKey = 'onyomi_kunyomi_highscore';
  ...
  Future<void> _loadHighScore() async {
    try {
      ...
      _highScore = prefs.getInt(_storageKey) ?? 0;
    } catch (_) {}
  }
  ...
  Future<void> _saveHighScore() async {
    try {
      final prefs = await SharedPreferences.getInstance();
      await prefs.setInt(_storageKey, _highScore);
    } catch (_) {}
  }
  ...
  Future<void> _regenerateKanji() async {
  ...
  void _nextQuestion() {
    ...
    if (_isOnyomi) {
      _currentReading =
          _currentKanji!.readingsOn[random.nextInt(_currentKanji!.readingsOn.length)];
    } else {
      _currentReading =
          _currentKanji!.readingsKun[random.nextInt(_currentKanji!.readingsKun.length)];
    }
    _feedback = null;
    setState(() {});
  }
  ...
  void _submitAnswer(bool guessIsOnyomi) {
    ...
    setState(() {});
    Future.delayed(
      Duration(milliseconds: _isCorrect ? 800 : 1500),
      () {
    ...
    return Card(
      child: Padding(
        ...
        child: Row(
          ...
          children: [
            _scoreItem(Icons.emoji_events, '$_score',
                scheme.tertiary, 'Score'),
            Container(width: 1, height: 32, color: scheme.outlineVariant),
            _scoreItem(Icons.trending_up, '$_highScore',
                AppColors.starColor, 'Best'),
            Container(width: 1, height: 32, color: scheme.outlineVariant),
          ...
          child: ElevatedButton(
            ...
            child: const Column(
              children: [
                ...
                Text('Onyomi', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
              ],
            ),
          ),
        ),
          ...
          child: ElevatedButton(
            ...
            child: const Column(
              children: [
                ...
                Text('Kunyomi', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
              ],
            ),
          ),
        ),
